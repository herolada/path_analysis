...
class PathAnalysis:
    ...
    def analyze_all_points(self):
        ...
        while goal_points:              # Main cycle.
            ...
            # First generate initial graph and get objects in its proximity.
            graph_points, points_line, _, _ = points_to_graph_points(start_point, goal_point, density=DENSITY, width=THRESHOLD)
            ...
            t = time.time()
            RESERVE = 20
            reduced_objects = self.get_reduced_objects(np.min(graph_points_arr[:,0]),np.max(graph_points_arr[:,0]),np.min(graph_points_arr[:,1]),np.max(graph_points_arr[:,1]),reserve=RESERVE)
            print("get reduced objects: {}".format(round(time.time() - t,2)))
            t = time.time()

            # Make sure that start and goal points are both valid (ie. not inside a building or so).
            # If not find new start/goal alongside the points_line (line of points between start and goal).
            # If can't find try other points around the start.
            start_index = 1
            found_valid_start = False
            while not self.is_point_valid(start_point,reduced_objects):
                if start_index >= len(points_line)-1:  # start == point before goal
                    graph_points_matrix = np.reshape(graph_points, (int(len(graph_points)/len(points_line)), len(points_line)))
                    graph_points_matrix = graph_points_matrix.T
                    for column in graph_points_matrix:
                        for point in column:
                            if point not in points_line:
                                start_point = point
                                if self.is_point_valid(start_point,reduced_objects):
                    ...
                    start_point = points_line[start_index]
                    start_index += 1
            
            if not self.is_point_valid(start_point,reduced_objects):
                start_point = goal_point
                continue

            goal_index = -1
            found_valid_goal = False
            while not self.is_point_valid(goal_point,reduced_objects):
                if goal_index <= -len(points_line)+1:  # goal == point after start, go wide now
                    graph_points_matrix = np.reshape(graph_points, (int(len(graph_points)/len(points_line)), len(points_line)))
                    graph_points_matrix = graph_points_matrix.T
                    for column in graph_points_matrix:
                        for point in column:
                            if point not in points_line:
                                start_point = point
                                if self.is_point_valid(goal_point,reduced_objects):
                    ...
                    goal_point = points_line[goal_index]
                    goal_index -= 1

            if not self.is_point_valid(goal_point,reduced_objects):
                continue

            # We have a valid start and goal. Now check if there exists a path between them.
            # Generate new points, but keep the old reduced objects (efficiency).
            graph_points, points_line, start_index, goal_index = points_to_graph_points(start_point, goal_point, density=DENSITY, width=THRESHOLD)
            graph_points_arr = np.array([[p.x,p.y,i] for i,p in enumerate(graph_points)])
            mask = [False] * len(graph_points)

            for k,graph_point in enumerate(graph_points):
                mask[k]  = self.is_point_valid(graph_point, reduced_objects)
            ...
            if shortest_path_vertices:
                pass
                """  # We have a valid start and goal. Now if there is an obstacle between the two,
                    # make a new goal from the point right before the obstacle and in line of sight of start.
                    points_line_valid = np.zeros(points_line.shape)[start]
                    for i,point in enumerate(points_line):
                        points_line_valid[i] = self.is_point_valid(point,reduced_objects)
                    
                    if not points_line_valid.sum() == len(points_line_valid):   # There is an obstacle.
                        new_goal_index = np.where(a==False)[0][0] - 1
                    else:
                        pass
                """

            graph_points, points_line, start_index, goal_index = points_to_graph_points(current_point, next_point, density=DENSITY, width=THRESHOLD)
            graph_points_arr = np.array([[p.x,p.y,i] for i,p in enumerate(graph_points)])
